...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Store unique fingerprint IDs for this employee-date combination
        if (!in_array($row['fingerprint_id'], $attendance_data[$key]['fingerprint_ids'])) {
            $attendance_data[$key]['fingerprint_ids'][] = $row['fingerprint_id'];
        }
    }
    ...
    if ($total_records > 0) {
        ...
        // Show specific employee ID for individual reports
        if ($report_type === 'individual' && !empty($employee_ID)) {
            echo "<p><strong>Employee ID:</strong> " . htmlspecialchars($employee_ID) . "</p>";
        }
        
        // Show division and section details for group reports
        if ($report_type === 'group') {
            if (!empty($division)) {
                // Get division name
                $div_name_query = "SELECT division_name FROM divisions WHERE division_id = ?";
                $div_stmt = mysqli_prepare($connect, $div_name_query);
                ...
                echo "<p><strong>Division:</strong> " . htmlspecialchars($div_row['division_name']) . "</p>";
                mysqli_stmt_close($div_stmt);
            } else {
                echo "<p><strong>Division:</strong> All Divisions</p>";
            }
            
            if ($section != 'all' && !empty($section)) {
                // Get section name
                $sec_name_query = "SELECT section_name FROM sections WHERE section_id = ?";
                $sec_stmt = mysqli_prepare($connect, $sec_name_query);
                mysqli_stmt_bind_param($sec_stmt, "s", $section);
                mysqli_stmt_execute($sec_stmt);
                $sec_result = mysqli_stmt_get_result($sec_stmt);
                $sec_row = mysqli_fetch_assoc($sec_result);
                echo "<p><strong>Section:</strong> " . htmlspecialchars($sec_row['section_name']) . "</p>";
                mysqli_stmt_close($sec_stmt);
            } else {
                echo "<p><strong>Section:</strong> All Sections</p>";
            }
        }
        ...
        echo "<h4><i class='fas fa-print me-2'></i>Print Report</h4>";
        ...
        echo "<th><i class='fas fa-fingerprint me-1'></i>Fingerprint IDs</th>";
        echo "<th><i class='fas fa-sign-in-alt me-1'></i>Check-In Times</th>";
        echo "<th><i class='fas fa-sign-out-alt me-1'></i>Check-Out Times</th>";
        echo "</tr></thead>";
        echo "<tbody>";
        ...
        foreach ($attendance_data as $record) {
            ...
            // Fingerprint IDs
            echo "<td>";
            if (!empty($record['fingerprint_ids'])) {
                foreach ($record['fingerprint_ids'] as $fp_id) {
                    echo "<span class='badge fingerprint-badge me-1'>FP: " . $fp_id . "</span>";
                }
            } else {
                echo "<span class='text-muted'>No fingerprint data</span>";
            }
            echo "</td>";
            ...
            if (!empty($record['check_in_times'])) {
                foreach ($record['check_in_times'] as $index => $check_in) {
                    echo "<span class='status-badge status-in d-block mb-1'>" . $check_in . "</span>";
                }
            } else {
            ...
            if (!empty($record['check_out_times'])) {
                foreach ($record['check_out_times'] as $index => $check_out) {
                    echo "<span class='status-badge status-out d-block mb-1'>" . $check_out . "</span>";
                }
            } else {
