...
try {
    // First, check if employee exists in main employees table
    $simple_sql = "SELECT employee_ID, employee_name FROM employees WHERE employee_ID = ?";
    $simple_stmt = $connect->prepare($simple_sql);
    ...
    $simple_stmt->bind_param("s", $employee_id);
    $simple_stmt->execute();
    $simple_result = $simple_stmt->get_result();
    
    if ($simple_row = $simple_result->fetch_assoc()) {
        // Employee found, now get additional details
    ...
    error_log("Error in get_employee_details.php: " . $e->getMessage());
    echo json_encode([
        'success' => false, 
